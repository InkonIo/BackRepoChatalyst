...
public class BotService {
    ...
    public Bot createBot(CreateBotRequest createBotRequest, Long userId) {
        ...
        // 2. Проверяем лимиты на создание бота
        if (!quotaService.canCreateBot(userId)) {
            throw new RuntimeException("У вас нет активного тарифа для создания бота. Пожалуйста, приобретите тариф.");
        }

        // 3. Проверяем, не существует ли уже бот с таким botIdentifier или accessToken
